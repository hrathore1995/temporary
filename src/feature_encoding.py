...
def draw_clusters(image, boxes, clusters):
    ...
    for idx, box in enumerate(boxes):
        ...
        cv2.putText(output, f"C{cluster_id}",
                    (x1, y1 - 3), cv2.FONT_HERSHEY_SIMPLEX, 0.3, color, 1)
...
def process_image(yolo_model, clip_model, preprocess, clip, img_path):
    ...
    cv2.putText(output, f"Products: {len(boxes)} | Encoded: {len(embeddings)} | Clusters: {num_clusters}",
