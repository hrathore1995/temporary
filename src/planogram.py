...
def draw_full_analysis(image,boxes,assigned_rows,row_means,gaps,nodes,ideal_nodes,score,violations):
    ...
    for row_idx,box_indices in assigned_rows.items():
        ...
        cv2.putText(output,f"R{row_idx+1}",(5,row_y-4),cv2.FONT_HERSHEY_SIMPLEX,0.4,color,1)
    ...
    for gap in gaps:
        ...
        cx=(gap["x1"]+gap["x2"])//2
        cy=(gap["y1"]+gap["y2"])//2
        cv2.putText(output,"OOS",(cx-15,cy),cv2.FONT_HERSHEY_SIMPLEX,0.45,(255,255,255),1)
    ...
    cv2.putText(output,f"RetailAudit-Net | Products: {len(boxes)} | Rows: {len(row_means)} | Gaps(OOS): {len(gaps)}",(10,25),cv2.FONT_HERSHEY_SIMPLEX,0.6,(255,255,255),1)
    cv2.putText(output,f"Planogram Compliance: {compliance_pct}% | Violations: {len(violations)}",(10,58),cv2.FONT_HERSHEY_SIMPLEX,0.6,score_color,2)
    return output
def process_image(yolo_model,clip_model,preprocess,img_path):
    ...
    output=draw_full_analysis(image, boxes, assigned_rows, row_means,gaps, nodes, ideal_nodes, score, violations)
