...
def draw_output(image,boxes,assigned_rows,row_means,gaps,score,violations,embeddings):
    output=image.copy()
    h,w=output.shape[:2]
    colors=[(255,80,80),(80,255,80),(80,80,255),(255,200,0),(0,200,255),(200,0,255),(255,128,0),(0,255,128)]
    for row_idx,box_indices in assigned_rows.items():
        color=colors[row_idx%len(colors)]
        row_y=int(row_means[row_idx])
        cv2.line(output,(0,row_y),(w,row_y),color,1)
        cv2.putText(output,f"Row {row_idx+1} ({len(box_indices)} items)",(5, row_y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1)
    ...
    for gap in gaps:
        ...
        cx=(gap["x1"]+gap["x2"])//2
        cy=(gap["y1"]+ gap["y2"])//2
        cv2.putText(output,"OOS",(cx-15,cy),cv2.FONT_HERSHEY_SIMPLEX, 0.5,(255,255,255),1)
    ...
    cv2.putText(output,"RetailAudit-Net | Full Pipeline",(10,22),cv2.FONT_HERSHEY_SIMPLEX,0.65,(255,255,255),1)
    cv2.putText(output,f"Phase 1: {len(boxes)} products detected | Phase 2: {len(row_means)} rows identified",(10,48),cv2.FONT_HERSHEY_SIMPLEX,0.5,(200,200,200),1)
    cv2.putText(output,f"Phase 3: {len(embeddings)} CLIP embeddings | Phase 4: Compliance {compliance_pct}% | OOS: {len(gaps)} | Violations: {len(violations)}",(10,74),cv2.FONT_HERSHEY_SIMPLEX,0.5,score_color,1)
    return output
def run_pipeline(img_path):
    ...
    output =draw_output(image, boxes, assigned_rows, row_means,gaps, score, violations, embeddings)
