...
def draw_shelf_analysis(image,boxes,row_means,horizontal_lines,assigned_rows):
    ...
    colors=[(255,0,0),(0,255,0),(0,0,255),(255,165,0),(128,0,128),(0,255,255),(255,20,147),(0,128,0)]
    for row_idx,row_boxes in assigned_rows.items():
        color=colors[row_idx%len(colors)]
        row_y= int(row_means[row_idx])
        cv2.line(output,(0,row_y),(w,row_y),color,2)
        cv2.putText(output,f"Row {row_idx + 1} ({len(row_boxes)} items)",(10,row_y-5),cv2.FONT_HERSHEY_SIMPLEX,0.5,color,1)
...
def draw_gaps(output,gaps,row_means,img_height):
    for row_idx,row_gaps in gaps.items():
        ...
        for gap in row_gaps:
            ...
            cv2.putText(output,"EMPTY",(x1+2,row_y),cv2.FONT_HERSHEY_SIMPLEX,0.4,(255,255,255),1)
    return output
def process_image(model,img_path):
    ...
    cv2.putText(output,f"Products: {len(boxes)} | Rows: {num_rows} | Gaps: {total_gaps}",(10,30),cv2.FONT_HERSHEY_SIMPLEX,0.7,(255,255,255),2)
    cv2.putText(output,f"Horizontal lines: {len(horizontal_lines)}",(10,60),cv2.FONT_HERSHEY_SIMPLEX,0.7,(255,255,255),2)
